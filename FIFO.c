                ...
                if (memoryProcesses == NULL) {
                    ...
                    if (availableMemory() >= (molduras)) { // Não precisa remover nenhum processo
                        ...
                        if (strcmp(politicaDeMemoria, "global") == 0) {
                            ...
                            // Caso oldest id seja igual a zero
                            if (newestProcess->id == oldestProcess->id){
                                initNewestProcess(0, 0, 0);
                            }

                            ProcessInMemory process;
                            process.id = listaP[posicao].id;
                            
                            //Fazer a memoria ficar circular
                            printf("+++++++++++++++++++++++++++TESTE\n ");
                            int firtKey = -1;
                            int secondKey = -1;
                            int availableUntillEnd = 0;
                            for(int i = 0; i < tamanhoMemoria/tamanhoPagina; i++){
                                if (primaryMemory[i] != 0){
                                    firtKey = primaryMemory[i];
                                }
                                if (primaryMemory[i] == 0 && firtKey == 0){ //conta o valor de molduras vazias até o final da memoria
                                    availableUntillEnd ++;
                                }
                            }

                            printf("availableUntillEnd %d - molduras %d\n",availableUntillEnd,molduras);
                            if(availableUntillEnd - molduras < 0) {
                                printf("Molduras nao suficientes até o final da memoria\n");
                            }
                            else{
                                if(newestProcess->end == 0){
